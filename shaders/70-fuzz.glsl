...
uniform mat4 u_mvp;
uniform float u_hull_offset;
uniform float u_hull_threshold;
uniform int u_lod;
...
vec3 cam_dir = (u_cam_pos-pos)/cam_dist;
setup_varyings(cam_dist);
float vanish = clamp(dot(cam_dir,v_normal)*3.0,0.0,1.0);
pos+=v_smooth_normal*u_hull_offset*vanish;
if((a_codes.w>>7)==0u){
pos=vec3(0);
}
